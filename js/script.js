        ...
        AnimationListener: function(event){
           ...
           if( (banner.config_REFER.bannerLoop_COUNT - 1) > banner.config_REFER.bannerLoop_CURRENT_COUNT){
              banner.config_REFER.bannerLoop_CURRENT_COUNT ++;
                ...
                // ROLLOVER event
                bannerInstance.addEventListener('mouseover',over)
                bannerInstance.addEventListener('mouseout',out)
...
function over(){
  var instance = document.getElementById('super1_over');
  var instance_idle = document.getElementById('label');

  instance_idle.style.opacity = 0;
  instance.classList.remove("rollover_reverse");
  instance.classList.add("rollover");
}
function out(){
  var instance = document.getElementById('super1_over');
  var instance_idle = document.getElementById('label');

  instance_idle.style.opacity = 1;
  instance.classList.add("rollover_reverse");
  instance.classList.remove("rollover");
}
